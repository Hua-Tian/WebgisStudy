...
;(function($) {
	...
	$.MessageBox = function(options,callback,target) {
		...
		modalbox.createDialog = function() {
			...
			modalbox.title = $('<h1/>', {'class':'title'});
			modalbox.span = $('<span/>', {'class':'glyphicon glyphicon-bullhorn'});
			modalbox.span.appendTo(modalbox.title);
			...
			modalbox.topCenter = 200;//(modalbox.windowHeight-modalbox.height)*0.50;
		...
		modalbox.insertDialog = function() {
			...
			if ( typeof parseInt(options.timeout.second) == 'number' && parseInt(options.timeout.second) > 0 ) {
				...
				modalbox.timeout.html(modalbox.remaining+"秒後に閉じます");
